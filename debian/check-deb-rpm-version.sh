...
case ${pkg} in
    ...
    jq|python-detach|python-jsonschema|python-psycopg2)
...
DEBIAN_VERSION=`perl -pe 's/^.* \(([0-9\.]+)(|\+dfsg|\+20[0-9]+|~[0-9]+(|\.(a|b|rc)[0-9]+))(\+20[0-9]+)?-[0-9\.]+(~bpo[0-9]+\+[0-9]+)?([+~]20[0-9]+)?\) .*$/$1/; last if $. > 1' < $changelog`
